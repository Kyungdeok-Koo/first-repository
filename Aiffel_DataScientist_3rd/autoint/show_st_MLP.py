...
import os
import joblib
from autoint_MLP import AutoIntMLPModel, predict_model
...
def load_data():
    ...
    # Tensor로 입력을 변환하여 모델을 호출
    model(tf.convert_to_tensor([[0 for _ in range(len(field_dims))]], dtype=tf.int32))  # Tensor 형식으로 바꿈
    
    model.build(input_shape=(None, len(field_dims)))  # 입력 차원에 맞춰 모델 빌드
    # 새 모델로부터 학습 후 가중치 저장
    model.save_weights('autoInt_model_weights.weights.h5')
    
    model.load_weights(f'{model_path}/autoInt_model_weights.weights.h5', skip_mismatch=True)
...
def get_recom(user, user_non_seen_dict, user_df, movies_df, r_year, r_month, model, label_encoders):
    ...
    '''
        ...
        # user_non_seen_dict가 None인 경우 처리
    if user_non_seen_dict is None:
        print("Error: user_non_seen_dict is None")
        return []  # None이면 빈 리스트 반환 (추천 결과가 없음)
    

    user_non_seen_movie = user_non_seen_dict.get(user)
    if user_non_seen_movie is None:
        print(f"Error: No unseen movies found for user {user}")
        return []  # 해당 사용자가 보지 않은 영화 목록이 없으면 빈 리스트 반환
