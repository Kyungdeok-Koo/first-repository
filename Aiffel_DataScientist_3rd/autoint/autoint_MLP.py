...
#     def __init__(self, field_dims, embedding_size, att_layer_num, att_head_num, att_res, dnn_hidden_units):
...
#         # embed_output_dim을 정의합니다.
#         self.embed_output_dim = embedding_size  # 임베딩 크기로 설정
...
class AutoIntMLP(Layer): 
    def __init__(self, field_dims, embedding_size, att_layer_num=3, att_head_num=2, att_res=True, dnn_hidden_units=(32, 32), dnn_activation='relu',
        ...
        self.embed_output_dim = embedding_size

        self.dnn = MultiLayerPerceptron(self.embed_output_dim, dnn_hidden_units,
                                        activation=dnn_activation, l2_reg=l2_reg_dnn, dropout_rate=dnn_dropout,
                                        use_bn=dnn_use_bn, output_layer=True)
        
        self.int_layers = [
            MultiHeadSelfAttention(self.embedding_size, att_head_num, att_res)
    ...
    def call(self, inputs, training=False):
        ...
        dnn_output = tf.reshape(dnn_output, [-1, 14])  # 차원 맞추기
        y_pred = tf.nn.sigmoid(att_output + dnn_output)
        
        return y_pred


# AutoIntMLP 레이어를 가지고 있는 모델 본체입니다. 해당 모델을 활용해 훈련을 진행합니다.
...
def predict_model(model, pred_df):
    ...
    for i in range(0, total_rows, batch_size):
        ...
        for feature, p in zip(features, y_pred):
            u_i = feature[:2]
            if isinstance(u_i[1], np.ndarray):
                # 다차원 배열인 경우
                if u_i[1].ndim > 1:
                   user_pred_info.append((float(u_i[1][0][0]), float(p)))  # 다차원 배열에서 첫 번째 값만 사용
                else:
                   user_pred_info.append((float(u_i[1][0]), float(p)))  # 1차원 배열에서 첫 번째 값만 사용
            else:
                user_pred_info.append((float(u_i[1]), float(p)))  # 일반적인 경우
